...
export default function JobsPage() {
  ...
  const load = async (loc = location) => {
    ...
    try {
      ...
      const resp = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/jobs/suggestions?location=${encodeURIComponent(loc)}&limit=20`,
  ...
  const filtered = jobs.filter(j => {
    if (filter === "high") return j.fit_score >= 70;
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-5 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <input value={location} onChange={e => setLocation(e.target.value)}
                onKeyDown={e => e.key === "Enter" && load(location)}
                placeholder="Location or Remote"
                className="px-2 py-1.5 text-xs bg-gray-900 border border-gray-700 rounded-lg text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-violet-500 w-36" />
              <button onClick={() => load(location)}
                className="px-2 py-1.5 text-xs bg-gray-800 text-gray-300 rounded-lg border border-gray-700 hover:bg-gray-700 transition-colors">
                Go
              </button>
            </div>
          ...
          {apiError ? (
            ...
            <div className="space-y-3">
              {filtered.map(job => (
                <JobCard key={job.job_id} job={job} scoreColor={scoreColor} />
              ))}
            </div>
          )}
        </>
...
function JobCard({ job, scoreColor }: { job: Job; scoreColor: (s: number) => string }) {
      ...
      <div className="flex items-center gap-2 mt-4 pt-3 border-t border-gray-800">
        ...
        <button onClick={() => setSaved(!saved)}
          className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
            saved ? "bg-violet-600/20 border-violet-500/30 text-violet-300"
